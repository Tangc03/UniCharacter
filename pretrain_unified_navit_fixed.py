...
#
# 修复版本: 使用修复后的 bagel_fixed.py 以避免 FSDP 死锁
# 修复时间: 2025-11-27
# 修复说明: 导入 bagel_fixed 模块中的 BagelConfig 和 Bagel 类
# 问题描述: 在 FSDP 模式下混合训练不同类型数据(带图VLM/不带图VLM/T2I)时,
#          不同 rank 的 batch 数据类型不一致会导致 FSDP 死锁
# 解决方案: 使用修复后的 Bagel 模型,确保所有 rank 都调用 vit_model
...
# ===== 修改导入: 使用修复后的 bagel_fixed 模块 =====
from modeling.bagel.bagel_fixed import BagelConfig, Bagel
...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        logger.info("=" * 80)
        logger.info("使用修复版本: bagel_fixed.py")
        logger.info("修复说明: 确保所有 rank 都调用 vit_model,避免 FSDP 死锁")
        logger.info("=" * 80)
        wandb.init(
    ...
    # Check the model path: print the tree structure of the model path(tree depth set to 1)
    if dist.get_rank() == 0:
        print("model path: ", model_args.model_path)
        print("model path tree: ", os.listdir("models/BAGEL-7B-MoT"))
    ...
    os.environ["nproc_per_node"] = str(training_args.num_shard)
    os.environ["WORLD_SIZE"] = str(training_args.num_shard)

    ema_model = deepcopy(model)

    # 如果resume_from是"models/BAGEL-7B-MoT"
    # print("resume from: ", resume_from)
    print("Loading model...")
    print("resume from: ", resume_from)
    if resume_from == "models/BAGEL-7B-MoT":
    ...
    print("Optimizer and scheduler setup done.")

    print("Setting up packed dataloader...")
    ...
    print("Packed dataloader setup done.")
    ...
    # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Starting training loop, about to iterate DataLoader...")
    for curr_step, data in enumerate(train_loader, start=train_step):
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Got batch from DataLoader for step {curr_step}")
        ...
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Moving batch to device {device}...")
        data = data.cuda(device)
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Converting to dict...")
        data = data.to_dict()
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Converted to dict, popping indexes...")
        data_indexes = data.pop('batch_data_indexes', None)
        ce_loss_weights = data.pop('ce_loss_weights', None)
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Starting forward pass...")
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            if training_args.visual_gen and 'padded_images' in data:
                # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Encoding images with VAE, image shape: {data['padded_images'].shape if 'padded_images' in data else 'N/A'}...")
                with torch.no_grad():
                    data['padded_latent'] = vae_model.encode(data.pop('padded_images'))
                # logger.info(f"[DEBUG] Rank {dist.get_rank()}: VAE encoding done, calling model...")
            else:
                # logger.info(f"[DEBUG] Rank {dist.get_rank()}: No T2I data in this batch, skipping VAE encoding...")
                pass
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Starting model forward...")
            loss_dict = fsdp_model(**data)
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Forward pass done, computing loss...")

        loss = 0
        ce = loss_dict["ce"]
        if ce is not None:
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Computing CE loss, ce_loss_indexes length: {len(data['ce_loss_indexes'])}")
            total_ce_tokens = torch.tensor(len(data['ce_loss_indexes']), device=device)
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: About to all_reduce total_ce_tokens...")
            dist.all_reduce(total_ce_tokens, op=dist.ReduceOp.SUM)
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: all_reduce total_ce_tokens done, value: {total_ce_tokens.item()}")
            if training_args.ce_loss_reweighting:
                ce = ce * ce_loss_weights
                total_ce_loss_weights = ce_loss_weights.sum()
                # logger.info(f"[DEBUG] Rank {dist.get_rank()}: About to all_reduce total_ce_loss_weights...")
                dist.all_reduce(total_ce_loss_weights, op=dist.ReduceOp.SUM)
                # logger.info(f"[DEBUG] Rank {dist.get_rank()}: all_reduce total_ce_loss_weights done")
                ce = ce.sum() * dist.get_world_size() / total_ce_loss_weights
            else:
                ce = ce.sum() * dist.get_world_size() / total_ce_tokens
            loss_dict["ce"] = ce.detach()
            loss = loss + ce * training_args.ce_weight
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: CE loss computed: {ce.item()}")
        else:
        ...
        if training_args.visual_gen:
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Computing MSE loss, mse_loss_indexes length: {len(data['mse_loss_indexes'])}")
            mse = loss_dict["mse"]
            total_mse_tokens = torch.tensor(len(data['mse_loss_indexes']), device=device)
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: About to all_reduce total_mse_tokens...")
            dist.all_reduce(total_mse_tokens, op=dist.ReduceOp.SUM)
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: all_reduce total_mse_tokens done, value: {total_mse_tokens.item()}")
            mse = mse.mean(dim=-1).sum() * dist.get_world_size() / total_mse_tokens
            loss_dict["mse"] = mse.detach()
            loss = loss + mse * training_args.mse_weight
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: MSE loss computed: {mse.item()}")
        else:
            assert not training_args.visual_gen
            loss_dict["mse"] = torch.tensor(0, device=device)
            total_mse_tokens = torch.tensor(0, device=device)

        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Starting backward pass...")
        optimizer.zero_grad()
        loss.backward()
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Backward pass done, clipping gradients...")
        total_norm = fsdp_model.clip_grad_norm_(training_args.max_grad_norm)
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Gradient clipping done, norm: {total_norm}")
        optimizer.step()
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Optimizer step done")
        scheduler.step()
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Scheduler step done")
        fsdp_ema_update(ema_model, fsdp_model, decay=training_args.ema)
        # logger.info(f"[DEBUG] Rank {dist.get_rank()}: EMA update done")

        # Log loss values:
        if curr_step % training_args.log_every == 0:
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: Logging step {curr_step}, preparing to all_reduce total_samples...")
            total_samples = torch.tensor(len(data['sample_lens']), device=device)
            dist.all_reduce(total_samples, op=dist.ReduceOp.SUM)
            # logger.info(f"[DEBUG] Rank {dist.get_rank()}: all_reduce total_samples done, value: {total_samples.item()}")
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            ...
            print(f"Checkpoint saved at {curr_step} in {training_args.checkpoint_dir}.")
