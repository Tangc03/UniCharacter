    ...
    def __iter__(self):
        ...
        while True:
            ...
            if sequence_status['curr'] == 0:
                if batch_count > 0:
                    # print(f"[DEBUG] Starting new batch (batch_count={batch_count})")
                    pass
                for group_index, group_iter in enumerate(self.dataset_iters):
                    if self.is_mandatory[group_index]:
                        # print(f"[DEBUG] Mandatory sampling from group {group_index}")
                        while True:
                            # print(f"[DEBUG] Calling next() on group {group_index} iterator")
                            sample = next(group_iter)
                            # print(f"[DEBUG] Got sample from group {group_index}, num_tokens={sample['num_tokens']}")
            ...
            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    ...
                    batch_count += 1
                    # print(f"[DEBUG] Batch {batch_count} yielded successfully")
                    yield data
                    # print(f"[DEBUG] After yield, continuing to next batch")
                    sequence_status = self.set_sequence_status()
                    batch_data_indexes = []
                continue
            ...
            if sequence_status['curr'] >= self.expected_num_tokens:
                ...
                data['batch_data_indexes'] = batch_data_indexes
                batch_count += 1
                # print(f"[DEBUG] Batch {batch_count} yielded (reached expected tokens)")
                yield data
                # print(f"[DEBUG] After yield, continuing to next batch")
                sequence_status = self.set_sequence_status()
                batch_data_indexes = []
