...
def motion_blur_opencv(image, kernel_size=15, angle=0):
    ...
    if img.ndim == 2:
        ...
        # For color images, each channel is convolved independently
...
def shuffle_patch(image, num_splits, gap_size=2):
    """
    Split the image into blocks (allowing for non-integer dimensions), 
    shuffle them randomly, 
    and then concatenate them, 
    leaving gaps between the blocks.
    """
    ...
    current_y = 0  # The starting Y coordinate of the current row
    patch_idx = 0  # The index of the current processed block
    for i in range(h_splits):
        current_x = 0  # The starting X coordinate of the current column
        patch_h = patch_heights[i]  # The height of the current block in the row
        for j in range(w_splits):
            # Get the shuffled block
            patch = patches[patch_idx]
            patch_w = patch_widths[j]  # The width of the current block in the column
            # Paste the block (the top-left corner is at (current_x, current_y))
            new_image.paste(patch, (current_x, current_y))
            # Update the X coordinate (the starting position of the next block = the width of the current block + the gap)
            current_x += patch_w + gap_size
            patch_idx += 1
        # Update the Y coordinate (the starting position of the next row = the height of the current row + the gap)
...
def inpainting(image, num_splits, blank_ratio=0.3, blank_color=(255, 255, 255)):
    """
    Split the image into blocks, randomly blank some patches, and then concatenate them, leaving gaps between the blocks.
    
    Args:
        image: PIL.Image Input image (RGB mode)
        h_splits: int Number of splits in the vertical direction (number of blocks in the vertical direction)
        w_splits: int Number of splits in the horizontal direction (number of blocks in the horizontal direction)
        blank_ratio: float The ratio of blank patches (0~1)
        blank_color: tuple The color of the blank region (RGB, e.g. white (255,255,255))
    
    Returns:
        PIL.Image The concatenated image after processing
    """
    h_splits, w_splits = num_splits
    img_w, img_h = image.size
    ...
    # Create the result image (the size is the same as the original image)
    ...
    for i in range(h_splits):
        ...
        patch_h = patch_heights[i]
        for j in range(w_splits):
            # Get the processed patch
            patch = processed_patches[patch_idx]
            patch_w = patch_widths[j]
            # Paste the patch to the original position
