...
class SftJSONLIterableDataset(DistributedIterableDataset):
    ...
    def __iter__(self):
        ...
        print(
            ...
            f"resuming data at row#{row_start_id}, total_paths={len(data_paths_per_worker) if data_paths_per_worker else 0}"
        )

        while True:
            data_paths_per_worker_ = data_paths_per_worker[row_start_id:]
            if row_start_id == 0:
                print(f"rank-{self.local_rank} worker-{worker_id} dataset-{self.dataset_name}: starting new epoch, {len(data_paths_per_worker_)} samples")
            for row_idx, (data, image_dir) in enumerate(data_paths_per_worker_, start=row_start_id):
                if row_idx % 20 == 0:
                    print(f"rank-{self.local_rank} worker-{worker_id} dataset-{self.dataset_name}: processing sample {row_idx}")
                num_tokens = 0
                image_tensor_list = []
                text_ids_list = []
                sequence_plan = []
