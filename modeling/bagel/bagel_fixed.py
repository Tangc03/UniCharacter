...
#
# Fix version: Implement solution 1 - unconditionally call VIT module to avoid FSDP deadlock
# Fix time: 2025-11-27
# Problem description: In FSDP mode, when the batch data types of different ranks are inconsistent (some have VIT data, some do not),
#           it will cause some ranks to call vit_model to trigger FSDP all-gather, while other ranks skip, causing deadlock
# Solution: Ensure that all ranks call vit_model, and use dummy forward when there is no VIT data
...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        # ===== Fix start: solution 1 - unconditionally call VIT module =====
        # Original code: if self.config.visual_und and vit_token_seqlens is not None:
        # Fix: ensure that all ranks call vit_model, avoid FSDP deadlock
        if self.config.visual_und:
            if vit_token_seqlens is not None:
                # Case 1: there is VIT data, normal processing
                ...
                # Case 2: there is no VIT data, create dummy call to trigger FSDP synchronization
                # Use smallest dummy input to reduce computational overhead
                # Note: the shape of packed_vit_tokens should be (num_patches, 3*patch_size*patch_size)
                # For patch_size=14, each patch is 3*14*14=588 dimensional
                dummy_tokens = torch.zeros(
                    1, 3 * 14 * 14,  # (1, 588) - the flattened shape of one patch
                    device=packed_text_embedding.device, 
                    dtype=packed_text_embedding.dtype
                )
                dummy_pos_ids = torch.zeros(
                    1, 
                    dtype=torch.long, 
                    device=packed_text_embedding.device
                )
                ...
                # Call vit_model through connector and vit_pos_embed
                # This ensures that all FSDP wrapped submodules are called synchronously
                packed_vit_token_embed = self.vit_model(
                    packed_pixel_values=dummy_tokens,
                    packed_flattened_position_ids=dummy_pos_ids,
                    cu_seqlens=dummy_cu_seqlens,
                    max_seqlen=1,
                )
                # Similarly, call connector and vit_pos_embed to ensure FSDP synchronization
                packed_vit_token_embed = self.connector(packed_vit_token_embed)
                vit_token_pos_emb = self.vit_pos_embed(dummy_pos_ids)
                packed_vit_token_embed = packed_vit_token_embed + vit_token_pos_emb
                
                # Key fix: let the result of dummy forward participate in the computation graph to ensure correct recomputation when backward
                # But does not affect the actual result (multiplied by 0)
                # This ensures that in the recomputation stage of gradient checkpointing,
                # all ranks will re-call these FSDP modules
                dummy_contribution = packed_vit_token_embed.sum() * 0.0
                # Add dummy_contribution to the first element of packed_sequence
                # This way it participates in the computation graph, but contributes 0
                packed_sequence[0] = packed_sequence[0] + dummy_contribution
        # ===== Fix end =====
